package com.example.chennuo.photowalldemo.Adapter;
...
/**
 * Created by chennuo on 2016/4/13.
 */
...
public class PhotoWallAdapter extends ArrayAdapter<String> implements AbsListView.OnScrollListener {
    ...
    //记录是否刚刚打开程序，用于解决进入程序不滚动屏幕，不会下载图片等的问题
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        //只有当GridView静止时才下载图片，滑动时取消现有的下载
     ...
     * @param mVisiableItemCount 屏幕中总共可见的元素数
     */
    public void loadBitmap(int mFirstVisibleItem, int mVisiableItemCount) {
        try {
            for (int i = mFirstVisibleItem; i < mFirstVisibleItem + mVisiableItemCount; i++) {
    ...
    class BitmapWorkerTask extends AsyncTask<String, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            //根据Tag找到相应的ImageView控件，将下载好的图片显现出来。
            ImageView imageView = (ImageView) mPhotoWall.findViewWithTag(imageurl);
            if (imageView != null && bitmap != null) {
                imageView.setImageBitmap(bitmap);
            }
